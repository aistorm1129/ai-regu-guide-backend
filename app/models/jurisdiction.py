from sqlalchemy import Column, String, DateTime, ForeignKey, Enum, Float, Text, UUID as SQLAlchemyUUID
...
class Jurisdiction(Base):
    ...
    requirements_data = Column(JSONB, nullable=True)  # Structured requirements data (renamed to avoid conflict)
    region = Column(String(100), nullable=True)  # e.g., "EU", "US", "Global"
    effective_date = Column(DateTime, nullable=True)
    
    # Assistant API integration
    assistant_id = Column(String(100), nullable=True)  # OpenAI Assistant ID
    vector_store_id = Column(String(100), nullable=True)  # OpenAI Vector Store ID
    source_file_id = Column(String(100), nullable=True)  # OpenAI File ID
    assistant_created_at = Column(DateTime, nullable=True)
    ...
    requirements = relationship("ComplianceRequirement", back_populates="jurisdiction")
